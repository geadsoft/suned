...
class VcQuarterlyStudent extends Component
{
    public $nivel,$subtitulo="",$alumno="",$materia="",$curso="";
    ...
    {   
       ...
       $this->tblparalelo = TmHorarios::query()
        ...
        return view('livewire.vc-quarterly-student');
    }

    public function consulta(){

        $persona = TmPersonas::find($this->filters['estudianteId']);
        ...
        $this->alumno =  $persona['apellidos'].' '.$persona['nombres'];
    ...
    public function add(){
        ...
        {   
            ...
            $this->tblrecords[$index]['prom70'] = 0.00;

            $this->tblrecords[$index]['promExamen'] = 0.00;
            $this->tblrecords[$index]['prom30'] = 0.00;
            $this->tblrecords[$index]['cuanti'] = 0.00;
            $this->tblrecords[$index]['cuali'] = "";

        }
        $this->tblrecords['ZZ']['id'] = 0;
        $this->tblrecords['ZZ']['asignaturaId'] = 0;
        ...
        $this->tblrecords['ZZ']['prom70'] = 0.00;

        $this->tblrecords['ZZ']['promExamen'] = 0.00;
    ...
    public function asignarNotas(){
        ...
        foreach ($examen as $asignaturaId => $objnota){

            if (isset($this->tblrecords[$asignaturaId]['promExamen'])) {
                $this->tblrecords[$asignaturaId]['promExamen'] = $objnota;
            }
        ...
        foreach ($materias as $key => $data) {
            ...
            if ($countprm > 0){
                ...
                $this->tblrecords[$data]['prom70'] = round(($promedio/($countprm))*0.70,2);
                $notafinal = $notafinal+$this->tblrecords[$data]['prom70'];
            }

            $this->tblrecords[$data]['prom30'] = round(floatval($this->tblrecords[$data]['promExamen'])*0.30,2);
            $notafinal = $notafinal+$this->tblrecords[$data]['prom30'];

            $this->tblrecords[$data]['cuanti'] = $notafinal;
            $this->tblrecords[$data]['cuali'] = "";
        ...
        foreach ($this->tblescala as $escala) {
            ...
            foreach ($this->tblrecords as $key => $record) {
                $promedio = $record['cuanti'];
                if ($promedio >= $nota1 && $promedio <= $nota2) {
                    $this->tblrecords[$key]['cuali'] = $letra;
                }
            }
