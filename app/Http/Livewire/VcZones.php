...
class VcZones extends Component
{
    use WithPagination;
    ...
    {
        $tblrecords = TmZonas::orderBy('root', 'asc')->paginate(10);        
        return view('livewire.vc-zones',['tblrecords' => $tblrecords]);
    }
    ...
    public function edit(TmZonas $tblrecords ){
    ...
    public function updateData(){
        ...
        if ($this->selectId){
            $record = TmZonas::find($this->selectId);
            $record->update([
                'descripcion' => $this -> record['descripcion'],
                'estado' => $this -> record['estado'],
            ]);
    ...
    public function deleteData(){
        TmZonas::find($this->selectId)->delete();
    ...
    public function setCodigo(){
        ...
        if ($this->superior==0){
            ...
            $this->ultcod = TmZonas::where('superior',0)->orderBy($this->sortData,$this->sortDirection)->first();
            ...
            $this->ultcod = TmZonas::where('superior',$this->superior)->orderBy($this->sortData,$this->sortDirection)->first();
