                        ...
                        <div class="mb-3">
                            ...
                            <table class="table table-bordered table-sm fs-12" id="orderTable">
                                <thead class="table-light">
                                    <tr><th colspan="{{ count($asignaturas) > 0 ? count($asignaturas) + 2 : 2 }}">
                                        ...
                                        @endif
                                        <p class="text-center text-uppercase" style="margin: 0px;">UNIDAD EDUCATIVA AMERICAN SCHOOL</p>
                                        <p class="text-center" style="margin: 0px;">CUADRO DE CALIFICACIONES FINALES CORRESPONDIENTE</p>
                                        <p class="text-center" style="margin: 0px;">PROMEDIO PARCIAL</p>
                                    ...
                                    <tr>
                                        <td class="align-middle text-center" style="font-weight: normal; padding: 0px 10px;">ALUMNO</td>
                                        @foreach($asignaturas as $key => $materia)
                                            <td class="align-middle text-center" style="font-weight: normal; padding: 2px 10px; writing-mode: vertical-lr; transform: rotate(180deg);">{{$materia->descripcion}}</td>
                                        @endforeach
                                        <td class="align-middle text-center" style="font-weight: normal; padding: 0px 10px;">Promedio</td>
                                    </tr>
                                </thead>
                                <tbody>
                                    ...
                                    <tr id="{{$fil}}" class="detalle">
                                        ...
                                        @foreach($asignaturas as $key => $materia)
                                            @if(isset($tblrecords[$record->id][$materia->id]['promedio']))
                                            <td class="align-middle text-center">{{number_format($tblrecords[$record->id][$materia->id]['promedio'],2)}}</td>
                                            @endif
                                        @endforeach
                                        @if(isset($tblrecords[$record->id]['ZZ']['promedio']))
                                        <td class="align-middle text-center">{{number_format($tblrecords[$record->id]['ZZ']['promedio'],2)}}</td>
                                        @endif
                                    </tr>
                                    @endforeach
                                </tbody>
